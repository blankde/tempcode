...
public:
	...
	void get_F1() {
		...
		if (minsup < 1)
			minsup = 1;
		fout << "DBASE_NUM_TRANS : " << TransDB().size() << endl;
		fout << "DBASE_MAXITEM : " << DBASE_MAXITEM << endl;
		fout << "MINSUPPORT : " << minsup << " (" << MINSUP_PER << ")"
				<< endl;
		...
		for (i = 0, j = 0; i < DBASE_MAXITEM; i++) {
			if (itcnt[it_order[i]] >= minsup) {
				if (output)
					fout << it_order[i] << " - " << itcnt[it_order[i]] << endl;
	...
	void get_F2(tqueue<SleuthTask>& task_queue) {
		int i, j;
		int it1, it2;
		int scnt;
	...
	virtual void setRoot(tqueue<SleuthTask>& task_queue){
		get_F1();
		get_F2(task_queue);
	}
